...
the Notary user agent string to the list of known notaries. In our implementation the notaries user agent string was:
...
When a server changes it's SSL certificate it sends a HTTP probe request to all
of the notaries it is aware of. In our implementation we extended the query
string of the HTTP GET requests that the perspective server. The probe request
