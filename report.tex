...
web-crawlers function. When a web server observes a notary checking its
certificate, it does so by checking the user-agent string of the notary's request.
By doing this the server knows how recent its SSL certificate entry is within the 
notary network system. 
